// main.js - Router principal (versión asíncrona para Firebase)
...
import './player.js';
import { auth } from './firebase.js'; // para asegurar que se inicie la autenticación
...
function updateActiveCategory() {
    const path = window.location.pathname;
    let activeCat = 'Todos';
    if (path.startsWith('/categoria/')) activeCat = decodeURIComponent(path.replace('/categoria/', ''));
    ...
    try {
        if (path === '/') {
            ...
            if (page) {
                ...
                if (buscarModule.renderCategory) buscarModule.renderCategory(container, cat);
                else {
                    const allEpisodios = await getAllEpisodios();
                    const categoryEpisodes = allEpisodios.filter(ep => ep.categories && ep.categories.includes(cat));
        ...
        container.innerHTML = `<div class="flex flex-col items-center justify-center min-h-[50vh] text-center px-4">
            ...
            <button onclick="window.location.href='/'" class="bg-[#7b2eda] hover:bg-[#8f3ef0] text-white font-bold px-6 py-3 rounded-full transition">Volver al inicio</button>
        </div>`;
    }
}
...
router();
console.log('✅ Main.js cargado con Firebase asíncrono');
